...
class DataLoader:
    ...
    def __iter__(self):
        ...
        if self.shuffle :
            index_iter = iter(np.random.permutation(len(self.dataset)))
        else :
            index_iter = iter(range(len(self.dataset)))
    ...
    def __len__(self):
        ...
        temp = int(len(self.dataset))%self.batch_size
        if self.drop_last==0 and temp>0:
            length+=1
